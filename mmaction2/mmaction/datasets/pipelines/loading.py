...
class SampleFrames:
    ...
    def __init__(self,
                 ...
                 frame_uniform=False): # frame_uniform is added by Swin
        ...
        self.frame_uniform = frame_uniform # added by Swin
    ...
    def __call__(self, results):
        ...
        # frame_uniform strategy is added by Swin
...
@PIPELINES.register_module()
class SampleUnlabelAVAFrames(SampleFrames):
    ...
    def __call__(self, results):
        ...
        center_index = results["timestamp"]
        ...
        # 为了 ssad 加的
        results["filename"] = f"{results['frame_dir']}/{center_index}"
        return results
...
class RawFrameDecodeWithGT:
    ...
    def __init__(self, gt_perform=False, io_backend='disk', decoding_backend='cv2', **kwargs):
        self.io_backend = io_backend
    ...
    def __call__(self, results):
        ...
        if self.gt_perform:
            ...
            if "former_gt_bboxes" in results:
                former_gt_bboxes = results["former_gt_bboxes"]
                former_gt_bboxes = (former_gt_bboxes * scale_factor).astype(np.float32)
                results["former_gt_bboxes"] = former_gt_bboxes
            
            if "later_gt_bboxes" in results:
                later_gt_bboxes = results["later_gt_bboxes"]
                later_gt_bboxes = (later_gt_bboxes * scale_factor).astype(np.float32)
                results["later_gt_bboxes"] = later_gt_bboxes
            
            filename = results["filename"]
            splits = filename.split("/")
            IND = str((int(splits[-1]) // 30 ) * 30 + 1)
            img_former = mmcv.imread(f"/mnt/disk1/suilin/ava/frames/{splits[-2]}/{splits[-2]}_{'0'*(6-len(IND))}{IND}.jpg")
            results["img_former"] = img_former
            
            IND = str((int(splits[-1]) // 30 + 1) * 30 + 1)
            img_later = mmcv.imread(f"/mnt/disk1/suilin/ava/frames/{splits[-2]}/{splits[-2]}_{'0'*(6-len(IND))}{IND}.jpg")
            results["img_later"] = img_later
