...
class AddPadShape:
    ...
    def __call__(self, results):
        results["pad_shape"] = (results["img_shape"][0], results["img_shape"][1])
        return results
...
class FilterGT:
    ...
    def __call__(self, results):
        # self.total_count += len(results["gt_bboxes"])
        # self.wrong_count += np.logical_or(results["gt_bboxes"][:, 0] == results["gt_bboxes"][:, 2], results["gt_bboxes"][:, 1] == results["gt_bboxes"][:, 3]).sum()
        # print(self.wrong_count / self.total_count)
        # exit()
        # bb, bl = results["gt_bboxes"].shape, results["gt_labels"].shape
        mask = np.logical_or(results["gt_bboxes"][:, 0] == results["gt_bboxes"][:, 2], results["gt_bboxes"][:, 1] == results["gt_bboxes"][:, 3])
        if mask.sum() == len(results["gt_bboxes"]):
            print("none...")
            results["gt_bboxes"] = np.zeros((0, 4))
            results["gt_labels"] = np.zeros((0, len(results["gt_labels"][0])))
            results["entity_ids"] = np.zeros(0)
        else:
            results["gt_bboxes"] = results["gt_bboxes"][mask]
            results["gt_labels"] = results["gt_labels"][mask]
            results["entity_ids"] = results["entity_ids"][mask]
        # print(bb, bl, results["gt_bboxes"].shape, results["gt_labels"].shape)
        # for i in ['frame_dir', 'video_id', 'timestamp', 'img_key', 'shot_info', 'fps', 'filename_tmpl', 'modality', 'start_index', 'timestamp_start', 'timestamp_end', 'entity_ids', 'frame_inds', 'clip_len', 'frame_interval', 'num_clips', 'crop_quadruple', 'imgs', 'original_shape', 'img_shape', 'scale_factor', 'keep_ratio', 'short_edge', 'crop_bbox']:
        #     print(i, results[i])
        # exit()
        return results
        # dict_keys(['frame_dir', 'video_id', 'timestamp', 'img_key', 'shot_info', 'fps', 
        # 'filename_tmpl', 'modality', 'start_index', 'timestamp_start', 'timestamp_end', 
        # 'gt_bboxes', 'gt_labels', 'entity_ids', 'frame_inds', 'clip_len', 'frame_interval', 
        # 'num_clips', 'crop_quadruple', 'imgs', 'original_shape', 'img_shape', 'scale_factor', 
        # 'keep_ratio', 'short_edge', 'crop_bbox'])
