...
class RandomCropWithGT:
    ...
    def __init__(self, size, lazy=False, gt_perform=False):
        if not isinstance(size, int):
            raise TypeError(f'Size must be an int, but got {type(size)}')
        self.size = size
        self.lazy = lazy
        self.gt_perform = gt_perform
    ...
    def _all_box_crop_former(self, results, crop_bbox):
        """Crop the gt_bboxes and proposals in results according to crop_bbox.
        ...
        Args:
            ...
            crop_bbox(np.ndarray): The bbox used to crop the original image.
        """
        results['former_gt_bboxes'] = self._box_crop(results['former_gt_bboxes'], crop_bbox)
        return results
    
    def _all_box_crop_later(self, results, crop_bbox):
        """Crop the gt_bboxes and proposals in results according to crop_bbox.
        ...
        Args:
            ...
            crop_bbox(np.ndarray): The bbox used to crop the original image.
        """
        results['later_gt_bboxes'] = self._box_crop(results['later_gt_bboxes'], crop_bbox)
        return results
    ...
    def __call__(self, results):
        ...
        if self.gt_perform:
            if "former_gt_bboxes" in results:
                assert not self.lazy
                results = self._all_box_crop_former(results, results['crop_bbox'])

            if "later_gt_bboxes" in results:
                assert not self.lazy
                results = self._all_box_crop_later(results, results['crop_bbox'])
            
            if "img_former" in results:
                results['img_former'] = self._crop_img(results['img_former'], crop_bbox)
            
            if "img_later" in results:
                results['img_later'] = self._crop_img(results['img_later'], crop_bbox)
...
class ResizeWithGT:
    ...
    def _resize_img(self, img, new_w, new_h):
        return mmcv.imresize(img, (new_w, new_h), interpolation=self.interpolation)
    ...
    def __call__(self, results):
        ...
        if 'former_gt_bboxes' in results:
            assert not self.lazy
            results['former_gt_bboxes'] = self._box_resize(results['former_gt_bboxes'],
                                                    self.scale_factor)
        
        if 'later_gt_bboxes' in results:
            assert not self.lazy
            results['later_gt_bboxes'] = self._box_resize(results['later_gt_bboxes'],
                                                    self.scale_factor)

        if "img_former" in results:
            results["img_former"] = self._resize_img(results["img_former"], new_w, new_h)

        if "img_later" in results:
            results["img_later"] = self._resize_img(results["img_later"], new_w, new_h)
...
class RandomRescaleWithGT:
    ...
    def __init__(self, scale_range, interpolation='bilinear', gt_perform=False):
        self.scale_range = scale_range
        # make sure scale_range is legal, first make sure the type is OK
        assert mmcv.is_tuple_of(scale_range, int)
        assert len(scale_range) == 2
    ...
    def __call__(self, results):
        ...
        short_edge = np.random.randint(self.scale_range[0],
                                       self.scale_range[1] + 1)
        if self.gt_perform:
            resize = ResizeWithGT((-1, short_edge),
                            keep_ratio=True,
                            interpolation=self.interpolation,
                            lazy=False)
        else:
...
class FlipWithGT:
    ...
    def _flip_img(self, img):
        _ = mmcv.imflip_(img, self.direction)
        return img
    ...
    def __call__(self, results):
        ...
        if self.gt_perform:
            if 'former_gt_bboxes' in results and flip:
                assert not self.lazy and self.direction == 'horizontal'
                width = results['img_shape'][1]
                results['former_gt_bboxes'] = self._box_flip(results['former_gt_bboxes'], width)
            
            if 'later_gt_bboxes' in results and flip:
                assert not self.lazy and self.direction == 'horizontal'
                width = results['img_shape'][1]
                results['later_gt_bboxes'] = self._box_flip(results['later_gt_bboxes'], width)
            
            if "img_former" in results and flip:
                results['img_former'] = self._flip_img(results['img_former'])
            
            if "img_later" in results and flip:
                results['img_later'] = self._flip_img(results['img_later'])
