...
class AVADatasetUnlabelGroup(BaseDataset):
    def __init__(
        self,
        ann_file_gt,
        pipeline,
    ...
    def load_annotations(self):
        ...
        self.gt_video_frame2anns = gt_video_frame2anns
        ...
        #     if former_multi_label is None and later_multi_label is None:
        #         pass 
        #     else:
        #         new_gt_video_infos.append(copy.deepcopy(video_info))
        self.gt_video_frame2multi_label = gt_video_frame2multi_label
        
        return gt_video_infos
    
    def prepare_train_frames(self, idx):
        results = copy.deepcopy(self.video_infos[idx])
        now_video_info = copy.deepcopy(self.video_infos[idx])
        #online part
        
        frame_dir, video_id, timestamp = now_video_info["frame_dir"], now_video_info["video_id"], now_video_info["timestamp"]

        current_multi_label = self.gt_video_frame2multi_label[video_id][timestamp] # 获取当前帧的 multi_label 信息
        ...
        current_gt_ann = self.gt_video_frame2anns[video_id][timestamp]
        current_gt_bboxes, current_gt_labels = current_gt_ann["gt_bboxes"], current_gt_ann["gt_labels"]
            ...
            # 向前找 multi_label 信息
        former_multi_label = self.gt_video_frame2multi_label[video_id].get(timestamp - 1, None)
        random_min_range = - self.max_range + 1
        random_max_range = self.max_range + 1
        if former_multi_label is not None:
            ...
            former_gt_ann = self.gt_video_frame2anns[video_id].get(timestamp -1 , None)
        ...
        later_multi_label = self.gt_video_frame2multi_label[video_id].get(timestamp + 1, None)
        if later_multi_label is not None:
            ...
            later_gt_ann = self.gt_video_frame2anns[video_id].get(timestamp + 1 , None)
        ...
        while True:
            frame_index = torch.randint(random_min_range, random_max_range, (1,)).item()
            if abs(frame_index) >= self.min_range:
            #if frame_index !=0:
                break 
        results["timestamp"] = center_index + frame_index
        results["img_key"] = f"{video_id},{results['timestamp']}"
        results["multi_label_restriction"] = multi_label_restriction    
        if frame_index < 0:
            results["former_gt_bboxes"] = former_gt_bboxes
            results["former_gt_labels"] = former_gt_labels
            results["later_gt_bboxes"] = current_gt_bboxes
            results["later_gt_labels"] = current_gt_labels
        else:
            results["former_gt_bboxes"] = current_gt_bboxes
            results["former_gt_labels"] = current_gt_labels
            results["later_gt_bboxes"] = later_gt_bboxes
            results["later_gt_labels"] = later_gt_labels
