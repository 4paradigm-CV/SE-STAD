...
class BBoxHeadAVA(nn.Module):
    ...
    def loss(self,
        ...
        if cls_score is not None:
            ...
            losses['loss_action_cls'] = self.loss_weight * torch.mean(F_loss)
...
class BBoxHeadJHMDB(nn.Module):
    ...
    def __init__(
        ...
        super(BBoxHeadJHMDB, self).__init__()
        assert temporal_pool_type in ['max', 'avg']
        assert spatial_pool_type in ['max', 'avg']
        self.temporal_pool_type = temporal_pool_type
        self.spatial_pool_type = spatial_pool_type
    ...
    def loss(self,
        ...
        # 不考虑背景的loss
        # if cls_score is not None:
        #     fg_idx = labels.sum(1) > 0
        #     labels = labels[fg_idx, 1:]
        #     cls_score = cls_score[fg_idx, 1:]
        #     label_gt = labels.nonzero()

        #     labels = label_gt[:, 1]
        #     ce_loss = F.cross_entropy # ce_loss 是自带 softmax 的
        #     loss = ce_loss(cls_score, labels)
        #     losses['loss_action_cls'] = self.loss_weight * loss

        # 考虑背景的loss
        if cls_score is not None:
            bg_idx = labels.sum(1) == 0
            labels[bg_idx, 0] = 1
            label_gt = labels.nonzero()
            assert len(label_gt) == len(labels), f"{labels}, {labels.shape}, {len(label_gt)}"
            labels = label_gt[:, 1]
            # print(labels.shape)
            cls_score = cls_score
    ...
    def get_det_bboxes(self,
        ...
        assert not self.multilabel
        # scores_index = cls_score.argmax(1) if cls_score is not None else None
        scores_index = cls_score[:, 1:].argmax(1) if cls_score is not None else None
        if scores_index is not None:
            scores = torch.zeros(cls_score.shape).to(cls_score)
            scores[list(range(len(scores_index))), (scores_index+1).long()] = cls_score[list(range(len(scores_index))), (scores_index+1).long()]
            # scores[list(range(len(scores_index))), (scores_index).long()] = cls_score[list(range(len(scores_index))), (scores_index).long()]
        bboxes = rois[:, 1:]
        assert bboxes.shape[-1] == 4
...
if mmdet_imported:
    ...
    MMDET_HEADS.register_module()(BBoxHeadJHMDB)