...
class SingleRoIExtractor3D(nn.Module):
    ...
    # 引入了 force_fp32 去完成 roi_extract 操作, 去支持自行自行实现的 fp16 的流程
    @force_fp32(apply_to=('feat', 'rois'), out_fp16=True)
    def forward(self, feat, rois):
        ...
        # 强制解决 force_fp32 不能将 tuple 中每个元素进行转换的问题
        if feat[0].dtype != rois.dtype:
            if len(feat) == 2:
                feat = (feat[0].to(torch.float), feat[1].to(torch.float))
            elif len(feat) == 1:
                feat = (feat[0].to(torch.float),)
            else:
                assert False, "length of feat is {} which is not supported now!".format(len(feat))
        ...
        for t in range(feat.size(2)):
            ...
            # print(frame_feat.dtype, rois.dtype)
